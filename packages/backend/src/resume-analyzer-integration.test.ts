...
%%EOF`;
    ...
    try {
      ...
      expect(Array.isArray(result.jobMatch?.matchingSkills)).toBe(true);
      expect(result.jobMatch?.matchingSkills.length).toBeGreaterThan(0);
      ...
      console.log(`\nMatching Skills (${result.jobMatch?.matchingSkills.length}):`);
      result.jobMatch?.matchingSkills.forEach(skill => {
        console.log(`  ✓ ${skill}`);
      });
